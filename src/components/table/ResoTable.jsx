...
const convertURLParamsToObj = (urlParams) => {
  try {
    return JSON.parse(
...
const ResoTable = (
  ...
  const paramsObj = convertURLParamsToObj(params.toString().substring(1));
  const form = useForm({
    defaultValues: { ...defaultFilters, ...paramsObj },
  });
  ...
  //     ..._filters,
  //   };
  //   navigate(`${location.pathname}?${queryString.stringify(queryStrings)}`);
  //   form.defaultFilters = queryStrings;
  // }, [current, pageSize, location.pathname, navigate, _filters, form]);
        ...
        {pagination && (
          <TablePagination
            ...
            {...{
              ...
              count: total,
              page: currentPage ? currentPage - 1 : current - 1,
            }}
            onPageChange={(_, page) => {
              setCurrentPage?.(page + 1);
              changeCurrent(page + 1);
            }}
